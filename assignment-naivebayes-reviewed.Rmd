...
```{r}
rawDF$label <- factor(rawDF$label, levels = c("0", "1"), labels = c("Reliable", "Unreliable")) %>% factor %>% relevel("Unreliable")
class(rawDF$label)
```
...
```{r}
cleanCorpus <- rawCorpus %>% tm_map(tolower) %>% tm_map(removeNumbers) %>% tm_map(removeWords, stopwords()) %>% tm_map(removePunctuation, preserve_intra_word_contractions = FALSE, preserve_intra_word_dashes = FALSE, ucp = TRUE) %>% tm_map(stripWhitespace)
```

Because we made a raw and clean corpus, we can now compare these to see if the cleanCorpus was made correctly.

```{r}
tibble(Raw = rawCorpus$content[1:10], Clean = cleanCorpus$content[1:10])
```
...
confusionMatrix(predVec, testDF$label, positive = NULL, dnn = c("Prediction", "True"))
```
...
Mostly the errors in this sabotaged code are just typos, as follow:

1. Line 39: 'rawDF' to 'ravDF'
2. Line 48: 'relevel("Unreliable")' to 'relevel(Umreliable)' 
3. Line 60: 'max.words = 20' to 'max.words = 2'
4. Line 70: 'VectorSource' to 'VectorSauce'
5. Line 71: 'rawCorpus' to 'rawCarp'
6. for line 85, author has removed 'tm_map(stripWhitespace)
7. Line 91: 'tibble' to 'pebble'
8. Line 98: 'inspect(cleanDTM)' to 'inspect(cleanDTN)'
9. Line 114 & 115: 'rawDF' to 'rawDE'
10. Line 176: 'testDF' to 'testTF'